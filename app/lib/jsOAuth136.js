        ...
        init: function (options) {
            ...
            this.request = function (options) {
                ...
                xhr.ondatastream = function(e){
                	Ti.API.debug("[jsOAuth.ondatastream]");
                	Ti.API.info("source: "+ e.source);
                	Ti.API.info("type: "+ e.type);
                	Ti.API.info("progress: " + e.progress);
                };
                xhr.onload = function(){
                	Ti.API.debug("[jsOAuth.onload]");
                };
                xhr.onerror = function(){
                	Ti.API.debug("[jsOAuth.onerror]");
                };
                xhr.onsendstream = function(){
                	Ti.API.debug("[jsOAuth.onsendstream]");
                };
                */
                /*
                if(options.stream){
                // if(false){
                	xhr.enableKeepAlive = true; // only IOS
					var last_index = 0;
					function parse() {
					    Ti.API.debug("[jsOAuth.parse] length:" + xhr.responseText.length);
					    // var curr_index = xhr.responseText.length;
					    // if (last_index == curr_index) return; // No new data
					    // var s = xhr.responseText.substring(last_index, curr_index);
					    // last_index = curr_index;
					    // Ti.API.debug("[jsOAuth.parse] response:" + s);
					    // Ti.API.debug(xhr.responseText);
					}
					// var interval = setInterval(parse, 5000);
					setTimeout(function(){
							var inputStream = Ti.Stream.createStream({
							    	mode:Ti.Stream.MODE_READ,
							    	source: xhr.responseData
							    });
